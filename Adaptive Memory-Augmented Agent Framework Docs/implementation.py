...
class MemoryGraph:
    ...
    def add_entity(
        ...
        self.graph.add_node(
            entity_id, type=entity_type, timestamp=timestamp, metadata=metadata or {}
        )
...
class MemoryManager:
    ...
    def add_memory(self, text: str, timestamp: float, embedding: np.ndarray) -> int:
        ...
        memory_entry = MemoryEntry(
            id=self.next_id, text=text, timestamp=timestamp, embedding=embedding
        )
    ...
    def update_memory_with_consolidation(
        self,
        memory_id: int,
        ...
        if memory_id in self.memory_store:
            ...
            operation_info = {
                "operation": "UPDATE(consolidate)",
                "memory_id": memory_id,
                "new_text": consolidated_text,
                "timestamp": new_timestamp,
            }
            self.operation_history.append(operation_info)
    ...
    def _detect_contradiction(self, new_text: str, existing_text: str) -> bool:
        ...
        for positive_terms, negative_terms in contradiction_patterns:
            if any(pos in existing_lower for pos in positive_terms) and any(
                neg in new_lower for neg in negative_terms
            ):
                return True
            if any(pos in new_lower for pos in positive_terms) and any(
                neg in existing_lower for neg in negative_terms
            ):
                return True
    ...
    def _consolidate_with_temporal_clause(
        self, old: MemoryEntry, new_text: str, new_ts: float
    ) -> str:
        ...
        if any(
            term in old.text.lower() or term in new_text.lower()
            for term in ["dairy", "vegetarian", "cheese"]
        ):
            return (
                f"Previously (as of {old_dt}), {old.text}. "
    ...
    def classify_and_execute_operation(
        self, text: str, timestamp: float, embedding: np.ndarray
    ) -> str:
        """
        ...
        if best_match_id is not None and best_similarity > 0.4:
            ...
            if self._detect_contradiction(text, best_match.text):
                ...
                consolidated_text = self._consolidate_with_temporal_clause(
                    best_match, text, timestamp
                )
                self.update_memory_with_consolidation(
                    best_match_id, consolidated_text, timestamp, embedding
                )
    ...
    def _extract_entities(self, text: str, timestamp: float):
        ...
        if "software engineer" in text_lower or "startup" in text_lower:
            self.memory_graph.add_entity(
                "Software_Engineer", "Occupation", np.random.rand(10), timestamp
            )
            self.memory_graph.add_entity("User", "Person", np.ones(10), timestamp)
            self.memory_graph.add_relationship(
                "User", "Software_Engineer", "works_as", timestamp
            )
...
class ACANRetrieval:
    ...
    def retrieve_relevant_memories(
        ...
        recency_weights = np.array(
            [self._recency_weight(mem.timestamp, current_time) for mem in memories]
        )
        importance_scores = np.array([mem.importance_score for mem in memories])

        composite_scores = (
            0.40 * attention_probs
            + 0.40 * cosine_similarities
        ...
        if apply_distillation:
            ...
            distillation_threshold = min(
                0.3, np.percentile(composite_scores, 25)
            )  # Use 25th percentile or 0.3, whichever is lower
            return self.memory_distillation(
                top_memories, top_scores, distillation_threshold
            )

        return top_memories

    def memory_distillation(
        self,
        memories: List[MemoryEntry],
        attention_scores: np.ndarray,
        distillation_threshold: float,
    ) -> List[MemoryEntry]:
...
class IntegratedMemoryAgent:
    ...
    def process_conversation_turn(
        self, user_message: str, assistant_response: str = None
    ) -> Dict:
    ...
    def answer_query(self, query: str, max_memories: int = 5) -> Dict:
        ...
        return {
            "query": query,
            "retrieved_memories": context_texts,
    ...
    def _generate_response(self, query: str, context_texts: List[str]) -> str:
        ...
        if any(term in query_lower for term in ["diet", "dietary", "eat", "food"]):
            memory = find_memory_with_keywords(
                "dairy", "vegetarian"
            ) or find_memory_with_keywords("cheese")
        ...
        if any(term in query_lower for term in ["pet", "dog"]):
            memory = find_memory_with_keywords(
                "dog", "named"
            ) or find_memory_with_keywords("dogs")
            if "two" in memory.lower() or (
                "buddy" in context_combined and "scout" in context_combined
            ):
        ...
        if any(term in query_lower for term in ["work", "job"]):
            ...
            if memory:
                return "Software engineer, startup."
        ...
        if any(term in query_lower for term in ["outdoor", "hiking", "activity"]):
            memory = find_memory_with_keywords("hiking") or find_memory_with_keywords(
                "mountain", "hiking"
            )
        ...
        return (
            context_texts[0][:100] + "..."
            if len(context_texts[0]) > 100
            else context_texts[0]
        )
...
def run_corrected_demo():
    ...
    for query in test_queries:
        ...
        print(
            f"Based on {result['num_memories_used']} memories: {result['memory_ids']}"
        )
