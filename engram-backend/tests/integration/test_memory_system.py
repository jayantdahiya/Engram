...
class TestMemorySystemIntegration:
    ...
    async def test_memory_consolidation_workflow(
        ...
        response3 = authenticated_client.post(
            "/memory/query",
            json={"query": "What are my dietary preferences?", "user_id": "test-user", "top_k": 5},
        )
        ...
        temporal_indicators = ["previously", "originally", "more recently", "now", "updated"]
    ...
    async def test_memory_retrieval_accuracy(
        ...
        for query, expected_keywords in test_queries:
            response = authenticated_client.post(
                "/memory/query", json={"query": query, "user_id": "test-user", "top_k": 3}
            )
            assert response.status_code == 200
    ...
    async def test_conversation_workflow(
        ...
        conv_response = authenticated_client.post(
            "/conversation/", json={"title": "Test Conversation", "user_id": "test-user"}
        )
    ...
    async def test_memory_statistics(
        ...
        test_memories = ["I am vegetarian", "I love hiking", "I work as a software engineer"]
    ...
    @pytest.mark.asyncio
    async def test_error_handling(self, authenticated_client: TestClient, db_session: AsyncSession):
        """Test error handling in memory operations"""

        # Test with invalid user ID
        response = authenticated_client.post(
            "/memory/query", json={"query": "Test query", "user_id": "invalid-user", "top_k": 5}
        )
        assert response.status_code == 200  # Should return empty results, not error

        # Test with empty query
        response = authenticated_client.post(
            "/memory/query", json={"query": "", "user_id": "test-user", "top_k": 5}
        )
